...
function Navbar({selectedProfile}) {
    ...
    const ClickBell= () => {
        document.getElementById('Bell').className='bell'
    }
    ...
    return (
        <div className={`container-navbar ${offset > 30 && `after-scroll`}`}>
            ...
            <div className='inner-container'>
                ...
                <h5 style={{ paddingLeft: 16, color: '#fff' }} className='mt-2'>{selectedProfile.title}</h5>
                <div style={{ color: '#fff', fontSize: 20, paddingLeft: 16 ,cursor:'pointer'}} onClick={ClickBell} id='Bell'><i className="fa fa-bell bell1"></i></div>
                <img onClick={navigateToProfile} width={40} style={{ objectFit: 'contain', borderRadius: 6, marginLeft: 16 ,cursor:"pointer"}} src={selectedProfile.asset} />
                <div style={{ color: '#fff', fontSize: 20, paddingLeft: 16,cursor:'pointer' }}>
                    <i className="fa fa-caret-down" onClick={navigateToProfile}></i>
                    </div>
            </div>
